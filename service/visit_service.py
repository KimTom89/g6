...
class VisitService:
    ...
    async def create_visit_record(self) -> Union[Visit, None]:
        """
        방문자 접속 이력 생성 함수
        - 새로운 접속 이력 생성
        - 방문자 합계 테이블 갱신
        ...
        if await self.is_exists_visit(vi_ip, self.today):
            return None
        ...
        await self._update_visit_sum()
        await self._update_config()

        return visit

    async def is_exists_visit(self, ip: str, visit_date: date) -> bool:
    ...
    async def _update_config(self) -> None:
        ...
        yesterday = await self.db.scalar(
            select(VisitSum.vs_count)
            .where(VisitSum.vs_date == self.today - timedelta(days=1))
        ) or 0
        visit_max = await self.db.scalar(func.max(VisitSum.vs_count)) or 0
        visit_total = await self.db.scalar(func.sum(VisitSum.vs_count)) or 0

        config = (await self.db.scalars(select(Config))).first()
        config.cf_visit = f"오늘:{today},어제:{yesterday},최대:{visit_max},전체:{visit_total}"
        await self.db.commit()

    async def _update_visit_sum(self) -> None:
        """방문자 합계 테이블 갱신 함수"""
        visit_count_today = await self.db.scalar(
